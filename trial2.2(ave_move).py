...
# 移動平均用の座標履歴（欠損防止用）
hand_coordinate_history = deque(maxlen=5)  # 過去5フレームの座標を保持
...
# 移動平均による座標補完関数（欠損防止用）
def get_smoothed_coordinates(current_x, current_y):
    global hand_coordinate_history
    
    # 現在の座標を履歴に追加
    hand_coordinate_history.append((current_x, current_y))
    
    # 移動平均を計算
    if len(hand_coordinate_history) > 0:
        x_coords = [coord[0] for coord in hand_coordinate_history]
        y_coords = [coord[1] for coord in hand_coordinate_history]
        smoothed_x = int(np.mean(x_coords))
        smoothed_y = int(np.mean(y_coords))
...
def predict_pose_from_video(source, model=None, le=None):
    global fish_position, hand_positions_history, hand_coordinate_history # 移動平均用履歴も追加
    ...
    while cap.isOpened():
        ...
        if handpose.size > 0:
            ...
            if predicted_gesture_cosine == 'paper' and landmarks:
                ...
                # 現在の手の座標をピクセルに変換
                raw_hand_x = int(middle_finger_base_landmark.x * frame.shape[1])
                raw_hand_y = int(middle_finger_base_landmark.y * frame.shape[0])
                
                # 移動平均による座標平滑化（欠損防止）
                current_hand_x, current_hand_y = get_smoothed_coordinates(raw_hand_x, raw_hand_y)
                ...
                hand_coordinate_history.clear() # 移動平均用履歴もクリア
        else: # 手が検出されない場合
            fish_position = None
