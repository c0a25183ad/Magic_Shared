...
def draw_movement_analysis(frame, width, height):
    ...
    # タイトル
    cv2.putText(frame, "Fish Movement Analysis (No Moving Average)", 
...
def predict_pose_from_video(source):
    ...
    while cap.isOpened():
        ...
        if hand_detected:
            ...
            # 生のランドマークを直接使用（移動平均なし）
            cursor_lm = raw_landmarks.landmark[mp_hands.HandLandmark.MIDDLE_FINGER_TIP]
            virtual_cursor_pos = (int(cursor_lm.x * width), int(cursor_lm.y * height))

            thumb_tip = raw_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
            pinky_tip = raw_landmarks.landmark[mp_hands.HandLandmark.PINKY_TIP]
            distance = np.sqrt((thumb_tip.x - pinky_tip.x)**2 + (thumb_tip.y - pinky_tip.y)**2)
            is_paper = distance > 0.15

            if is_paper:
                pos_lm = raw_landmarks.landmark[mp_hands.HandLandmark.MIDDLE_FINGER_MCP]
                current_hand_x, current_hand_y = int(pos_lm.x * width), int(pos_lm.y * height)
